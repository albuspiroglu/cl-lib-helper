...
(eval-when (:compile-toplevel :execute :load-toplevel)
  ...
  (unless (find-package "LIB")
    (lib~:setup-packages lib~:*lib-package-details*)
  ...
  (let ((pkg (find-package "LIB")))
    ...
    (let ((f (intern "GET-PACKAGE-NAMES" pkg)))
      (setf (symbol-function f)
            (lambda ()
              (lib~:get-package-names-aux lib~:*lib-package-details*))))
    (let ((f (intern "PACKAGES" pkg)))
      (setf (symbol-function f)
            (lambda (&key (stream *standard-output*)
                          (show-packages nil))
              (lib~:packages-aux lib~:*lib-package-details*
                                 :stream stream)))))

  )

(eval-when (:execute :load-toplevel)
  (push 'lib~:*lib-package-details* lib~:*package-lists*))
